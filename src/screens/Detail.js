...
export default function Detail({
  ...
  return (
    <Container backgroundColor="white" p={10} m={10}>
      <Link to="/">Volver</Link>
      {loading ? (
        <Text alignSelf="center" fontSize="2rem" fontWeight="bold">
          Cargando..
        </Text>
      ) : (
        <>
          <Text alignSelf="center" fontSize="4rem">
            {event.name}
          </Text>
          <Container flexDirection="row" width="100%">
            <Container flex={1}>
              <Container width="100%" p={10}>
                ...
                <Container flexDirection="row" width="100%">
                  <Text fontSize={24} fontWeight="900" flex={1}>
                    Fecha y hora
                  </Text>
                  <Text fontSize={24} flex={2}>
              ...
              <Container width="100%" p={10}>
                <Divider orientation="left">
                  Informacion del Recinto deportivo
                </Divider>
                <Container flexDirection="row" width="100%">
                  <Text fontSize={24} fontWeight="900" flex={1}>
                    Nombre del recinto
                ...
                <Container flexDirection="row" width="100%">
                  ...
                  <Text fontSize={24} flex={2}>
                    {event.SportsVenue.street} {event.SportsVenue.number}
                  </Text>
                </Container>
                <Container flexDirection="row" width="100%">
                  <Text fontSize={24} fontWeight="900" flex={1}>
                  ...
                  <Text fontSize={24} flex={2}>
                    {event.SportsVenue.commune}
                  </Text>
                </Container>
                <Container width="100%" border="1px solid">
                  <Map
                    center={[
                      event.SportsVenue.latitude,
                      event.SportsVenue.longitude,
                    ]}
                    zoom={13}
                  >
                    <TileLayer
            ...
            <Container flex={1} p={10}>
              ...
              <Text fontWeight="bold" fontSize="2rem">
                Faltan{` `}
                {event.GameType.name === `Fútbol`
                  ? 22 - players.length
                  : event.GameType.name === `Futbolito`
                  ? 14 - players.length
                  : 10 - players.length}
                {` `}
                Jugadores
              ...
              <Container flexDirection="row" width="100%" m="10px 0">
                <Input
                  placeholder="Email"
                  value={playerMail}
                  onChange={({ target: { value } }) => setPlayerMail(value)}
                />
                <Button disabled={!playerMail} onClick={() => sendInvitation()}>
                  Invitar
                </Button>
              </Container>
              <Table
                ...
                dataSource={players}
                onRowClick={({ playerMail }) =>
                  confirm({
                    title: `¿Aceptar invitación al partido?`,
                    okText: `Aceptar`,
                    cancelText: `Rechazar`,
                    onOk: async () => {
                      await API.updateEventInvitation(id, {
                        playerMail,
                        status: `Aceptado`,
                      })
